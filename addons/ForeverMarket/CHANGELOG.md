...
- Replaced the ad-hoc AuctionHouseFrame mode switch with a dedicated native Auction House display tab bridge built on Blizzard's AuctionHouseFrameDisplayModeTabTemplate.
- Modern Sell now calls PostItem/PostCommodity directly from the original Post auction button click (no intermediate custom confirm), preserving the hardware click path used by modern AH addons.
- Added ADDON_ACTION_BLOCKED / ADDON_ACTION_FORBIDDEN diagnostics plus secure-path logging around the post call.
- Keeps the proven debug5 sale parameters: stored locked ItemLocation, whole-silver Mainline price normalization, correct duration/quantity and CachePendingPost fallback.
...
## 2.4.0-debug7 - Native AH context + direct posting path
...
## 2.4.0-debug7 - Selling path rebuild
