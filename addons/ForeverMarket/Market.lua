local _,FM=...
local M={results={},owned={},generation=0,open=false,page=0,total=0,ownerPage=0};FM.Market=M
...
function M:ReadRow(kind,index)
    ...
    if not name or not count or count<1 then return nil end
    ...
    return {index=index,name=name,texture=texture,count=count,quality=quality or 1,minBid=minBid or 0,increment=increment or 0,
        buyout=buyout or 0,bid=bid or 0,owner=fullOwner or owner or "?",link=link,key=FM:ItemKey(link,name),
        unit=buyout and buyout>0 and buyout/count or 0,complete=complete~=false and link~=nil and (kind=="owner" or owner~=nil or fullOwner~=nil),
