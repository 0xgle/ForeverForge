...
function D:SnapshotSell(stage)
    ...
    local location
    if ItemLocation and type(ItemLocation.CreateFromBagAndSlot) == "function" and item.bag ~= nil and item.slot ~= nil then
        local ok, loc = pcall(ItemLocation.CreateFromBagAndSlot, ItemLocation, item.bag, item.slot)
        if ok then location = loc else self:Log("LOCATION_ERROR", loc) end
    end
    self:Log("LOCATION", "created="..bool(location ~= nil), "value="..tostring(location))
...
D:Log("DEBUG_BOOT", "ForeverMarket="..tostring(FM.version), "debug4")
