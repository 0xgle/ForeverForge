...
function U:RefreshSellSummary()
    ...
    if self.sellPost then
        local ready=v~=nil and M.open and not M.transaction
        if FM:IsModernAH() then ready=ready and S:IsPostReady() end
        self.sellPost:SetEnabled(ready)
    end
    if not v then self.sellSummary:SetText(err or "Select an item from your bags.");return end
    local total=FM:IsModernAH() and (v.unit*(v.quantity or 1)) or (v.buyout*v.count);local net=math.floor(total*(1-FM.DB.settings.cut))
    local serverState=""
    if FM:IsModernAH() then serverState=S:IsAHReadyForPost() and "  |  SERVER READY" or "  |  SERVER BUSY" end
    self.sellSummary:SetText("Stack: "..FM:Money(v.buyout).."  |  Total: "..FM:Money(total)..serverState.."\nDeposit: "..FM:Money(v.deposit).."  |  After fees: ~"..FM:Money(net).."\n"..v.count.." stacks x "..v.stack.." items  /  "..({12,24,48})[v.duration].." h")
end
...
function U:BuildConfirm()
    ...
    T:Button(p,"Back",154,34,206,186,function() U:DismissConfirm(false) end)
    self.confirmAccept=T:Button(p,"Confirm",154,34,376,186,function()
        local fn=U.confirmAction;U:DismissConfirm(true);if fn then fn() end
    end,true)
end
function U:Confirm(title,row,text,fn,cancelFn)
    self:Build();self.confirmTitle:SetText(title);self.confirmText:SetText(text);self.confirmAction=fn;self.confirmCancel=cancelFn;self.confirm:Show()
end
function U:DismissConfirm(accepted)
    local cancelFn=self.confirmCancel
    self.confirmAction=nil;self.confirmCancel=nil
    if self.confirm then self.confirm:Hide() end
    if not accepted and cancelFn then cancelFn() end
end
function U:Refresh()
...
local function AddForeverMarketTooltip(tip)
    if not tip or not FM.DB or not FM.DB.settings or not FM.DB.settings.tooltip then return end
    if type(tip.GetItem)~="function" then return end

    local ok,name,link=pcall(tip.GetItem,tip)
    if not ok or not link then return end

    local key=FM:ItemKey(link,name)
    local h=FM:GetHistoryStats(key)
    local src=FM.Price:Sources(key,link)
    if h then
        tip:AddLine("ForeverMarket | market: "..FM:Money(h.median).." / item",.6,.84,.78)
        tip:AddLine("Recent low: "..FM:Money(src.FMMinBuyout or h.low).." | "..h.count.." samples | "..math.floor((time()-h.t)/60).." min",.6,.66,.67)
    end
    local buy,sell=FM.Ledger:Averages(key)
    local owned=FM.Ledger:OwnedEverywhere(key)
    local _,g=FM.Groups:GroupFor(key)
...
FM:On("PLAYER_LOGIN",function()
    if not FM:Supported() or not GameTooltip then return end

    -- Modern WoW/Forever tooltips no longer expose OnTooltipSetItem as a
    -- hookable frame script. Use the supported post-call processor when it
    -- exists (the same API family used by current AH addons).
    if TooltipDataProcessor and type(TooltipDataProcessor.AddTooltipPostCall)=="function"
        and Enum and Enum.TooltipDataType and Enum.TooltipDataType.Item then
        local ok=pcall(TooltipDataProcessor.AddTooltipPostCall,Enum.TooltipDataType.Item,function(tip)
            AddForeverMarketTooltip(tip)
        end)
        if ok then return end
    end

    -- Legacy fallback. Some clients still expose OnTooltipSetItem; never let
    -- an unsupported script type prevent ForeverMarket from loading.
    if type(GameTooltip.HookScript)=="function" then
        pcall(GameTooltip.HookScript,GameTooltip,"OnTooltipSetItem",function(tip)
            AddForeverMarketTooltip(tip)
        end)
    end
end)
